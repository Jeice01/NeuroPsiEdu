...
import { Stethoscope, GraduationCap, ArrowRight, BrainCircuit, HeartPulse, ShieldCheck } from "lucide-react";

export function DualEntryCards() {
  return (
    <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12 relative z-20">
      {/* Patient Card */}
      <motion.div
      ...
      <motion.div
        ...
        className="bg-white/80 backdrop-blur-2xl border border-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_30px_60px_rgba(28,69,104,0.12)] rounded-[2rem] p-8 md:p-10 relative overflow-hidden group transition-all duration-500 hover:-translate-y-2 md:col-span-2 lg:col-span-1"
      >
        {/* Subtle glow accent */}
        <div className="absolute inset-0 bg-gradient-to-br from-neuro-orange/3 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-[2rem]" />

        <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
          <ShieldCheck size={120} className="text-neuro-orange" />
        </div>

        <div className="relative z-10 flex flex-col h-full">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-neuro-orange/20 to-amber-500/10 flex items-center justify-center mb-6">
            <ShieldCheck className="w-7 h-7 text-neuro-orange" />
          </div>

          <h3 className="text-2xl font-display font-bold text-slate-900 mb-3">
            Quero Supervisão Clínica
          </h3>

          <p className="text-slate-600 mb-8 flex-grow">
            Supervisão especializada para psicólogos que desejam fortalecer análise clínica, raciocínio diagnóstico e condução de casos em Avaliação Neuropsicológica.
          </p>
          ...
          <ul className="space-y-2 mb-8">
            {[
              ...
              <li key={item} className="flex items-center gap-2 text-sm text-slate-600">
                <span className="w-1.5 h-1.5 rounded-full bg-neuro-orange shrink-0" />
                {item}
              </li>
            ))}
          </ul>
          ...
          >
            <span className="font-semibold text-slate-900 group-hover/btn:text-neuro-orange transition-colors">Quero conhecer a supervisão</span>
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm border border-slate-100 group-hover/btn:bg-neuro-orange group-hover/btn:border-neuro-orange group-hover/btn:text-white transition-all duration-300 group-hover/btn:scale-110">
              <ArrowRight className="w-5 h-5" />
            </div>
          </a>
        </div>
