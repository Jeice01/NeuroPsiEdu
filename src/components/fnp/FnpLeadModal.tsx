...
export function FnpLeadModal({ isOpen, onClose, botaoOrigem }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const message = await submit(payload, FORMATIONS.fanp.successMessage);
    if (!message) return;
    setSuccessMessage(message);
    pushLeadEvent({
      event: "lead_formacao", formacao: "FANP", pagina: "/fnp",
            ...
            <div className="flex-1 overflow-y-auto px-6 sm:px-8 py-6">
              {success ? (
                ...
                <form onSubmit={handleSubmit} noValidate className="space-y-5">
                  <HoneypotField id="fnp-website" value={honeypot} onChange={setHoneypot} />
